...
class ArrayTestsFragment : Fragment() {
    ...
    private fun setupToolbar() {
        activity?.title = getString(R.string.array_tests_screen_title)
    }
    ...
    private fun setupRecyclerView() {
            ...
            adapter = TextListAdapter(
                ...
                clickListener = object :
                    ...
                    override fun onItemClicked(item: Int) {
                        when (item) {
                            0 -> findNavController().navigate(R.id.arrayTestsToSystemSortingTest)
                            1 -> findNavController().navigate(R.id.arrayTestsToManualSortingTest)
                            2 -> findNavController().navigate(R.id.arrayTestsToMappingTest)
                            3 -> findNavController().navigate(R.id.arrayTestsToFilteringTest)
        ...
        adapter?.submitList(resources.getStringArray(R.array.array_menu).toMutableList())
    }
}