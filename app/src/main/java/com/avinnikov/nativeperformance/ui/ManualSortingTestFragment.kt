...
import com.avinnikov.nativeperformance.extensions.quickSort
...
@ExperimentalTime
class ManualSortingTestFragment : Fragment() {
    ...
    private fun setupToolbar() {
        activity?.title = getString(R.string.manual_sorting_test_screen_title)
    }
    ...
    private fun doMeasure(): String {
        ...
        repeat(ITERATIONS_COUNT) {
            ...
            times.add(measureTime { items.quickSort() }.inSeconds)
        }

        return getString(R.string.sort_time).format(times.average())
    }
}