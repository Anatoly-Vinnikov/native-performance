...
class StringTestsFragment : Fragment() {
    ...
    private fun setupToolbar() {
        activity?.title = getString(R.string.string_tests_screen_title)
    }
    ...
    private fun setupRecyclerView() {
            ...
            adapter = TextListAdapter(
                ...
                clickListener = object :
                    ...
                    override fun onItemClicked(item: Int) {
                        when (item) {
                            0 -> findNavController().navigate(R.id.stringTestsToConcatenationTest)
                            1 -> findNavController().navigate(R.id.stringTestsToComponentsTest)
        ...
        adapter?.submitList(resources.getStringArray(R.array.string_menu).toMutableList())
    }
}