...
class RecyclerTestsFragment : Fragment() {
    ...
    private fun setupToolbar() {
        activity?.title = getString(R.string.recycler_tests_screen_title)
    }
    ...
    private fun setupRecyclerView() {
            ...
            adapter = TextListAdapter(
                ...
                clickListener = object :
                    ...
                    override fun onItemClicked(item: Int) {
                        when (item) {
                            0 -> findNavController().navigate(R.id.recyclerTestsToLongTextTest)
                            1 -> findNavController().navigate(R.id.recyclerTestsToShortTextTest)
                            2 -> findNavController().navigate(R.id.recyclerTestsToImageTest)
                            3 -> findNavController().navigate(R.id.recyclerTestsToImageAlphaTest)
                            4 -> findNavController().navigate(R.id.recyclerTestsToDynamicTextTest)
        ...
        adapter?.submitList(resources.getStringArray(R.array.recycler_menu).toMutableList())
    }
}