...
@ExperimentalTime
class MappingTestFragment : Fragment() {
    ...
    private fun setupToolbar() {
        activity?.title = getString(R.string.mapping_test_screen_title)
    }
    ...
    private fun doMeasure(): String {
        ...
        repeat(ITERATIONS_COUNT) {
            ...
            times.add(measureTime { items.map { "${it.number}. ${it.string}" } }.inSeconds)
        }

        return getString(R.string.mapping_time).format(times.average())
    }
}